    ...
    fileTool.readDirectory(dir)
      .then(function(files) {
        ...
        files.forEach(function(file) {
          if ( file.fullPath.indexOf("index.html", file.fullPath.length - "index.html".length) === -1 ) { // All but index.html
            ui.writeLine(chalk.green('Added ' + file.fullPath + ' to queue'));
            queue.push({
		...
		files.forEach(function(file) {
          if ( file.fullPath.indexOf("index.html", file.fullPath.length - "index.html".length) !== -1 ) { // index.html last
            ui.writeLine(chalk.green('Added ' + file.fullPath + ' to queue'));
            queue.push({
              fullPath: file.fullPath,
              name: file.path,
...
module.exports.uploadFile = function(s3, ui, fullPath, params) {
  ...
  if (fileName == 'index.html' || fileName.indexOf('.html', fileName.length - '.html'.length) !== -1 ) {
    params.CacheControl = "no-cache, must-revalidate"; //max-age=29030400, public for caching if you want that
    ui.writeLine(chalk.green('No Cache for ' + fileName + ' set'));
  }

  if ( fullPath.indexOf('static-root') !== -1 ) {
    ui.writeLine(chalk.green('Base asset found for ' + fileName + ' path ' + fullPath));
    ui.writeLine(chalk.green('Key was ' + params.Key + ' now ' + fileName));
    params.Key = fullPath.substring(fullPath.indexOf('static-root')+12);
  }
